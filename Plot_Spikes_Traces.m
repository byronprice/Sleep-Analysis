function [] = Plot_Spikes_Traces(Av_Data,Spike_Data,Trace_Data)
%Plot_Spikes_Traces.m
% Created: 2015/12/11 at 24 Cummington, Boston
%   Byron Price
%  plotSpikeRaster was made by Jeffrey Chiou ... see below
% Updated: 2015/12/11
% By: Byron Price
%   This file will run through detrended fluorescence traces, spike data, 
%    and summed spike data across ROIs, creating a single figure for each
%    video. It assumes a sampling frequency of 30 Hz.
%
%    Call this file as Plot_Spikes_Traces( ... );  with no output
% INPUT:   Av_Data - number videos by number frames matrix of summed spike
%                       activity
...
for i=1:numVideos
    ...
    spikes = squeeze(Spike_Data(i,:,:));
    traces = squeeze(Trace_Data(i,:,:));
    ...
    axis([0 Recording_Time 0 max(avalanches)+5])
    ...
    %set(gca,'XTick',0:Recording_Time/6:Recording_Time)
...
function  Fluorescence_Plot(Input_Data,times,Recording_Time)

numROIs = size(Input_Data,1);
c = colormap(lines(numROIs)); 
...
for i = numROIs:-1:1
    ...
    shiftup = (3*count);
    plot(times,z+shiftup,'Color',c(count,:),'LineWidth',1);
    count = count+1;
end
axis([0 Recording_Time 0 shiftup+3])
hold off
end
